...
## Overview
Reproducible build tooling that produces a conventionally named plugin binary for all platforms the Infracost CLI supports.
...
- Output naming follows the registry convention: `infracost-parser-plugin-expectedload` (with `.exe` appended on Windows). Per-platform artifacts are disambiguated by directory or archive name. The CLI discovers type and identity through `GetPluginInfo`, not the filename.
...
- Debug builds use the `-debug` suffix (`infracost-parser-plugin-expectedload-debug`) to keep them distinct from release artifacts. Do not place both binaries in the CLI plugin directory: every executable there is probed.
